...
from transformers import AutoConfig, AutoModelForCausalLM, AutoTokenizer, EarlyStoppingCallback, HfArgumentParser, \
	Trainer, TrainerCallback, TrainerControl, TrainerState, TrainingArguments, \
	default_data_collator, set_seed

from modeling_olmo_dp import OlmoForDiagnosticProbing
from util.const import IS_UNARY, MAX_TARGET, OLMO_1B_HF_PATH
from util.dp_arguments import DataTrainingArguments, OlmoArguments
...
parser = HfArgumentParser((OlmoArguments, DataTrainingArguments, TrainingArguments))
model_args, data_args, training_args = parser.parse_args_into_dataclasses()
model_args: OlmoArguments
...
# Post-process args
model_args.model_path = OLMO_1B_HF_PATH
post_process_training_args(training_args=training_args, wandb_proj_name=wandb_proj_name, serial=serial)

# Load OLMo config
olmo_config_kwargs = {
	"cache_dir"     : model_args.cache_dir,
	"revision"      : model_args.branch,
	"use_auth_token": True,
	"use_cache"     : True,
	}
olmo_config = AutoConfig.from_pretrained(model_args.model_path, **olmo_config_kwargs)
label2id, id2label = get_label_and_id_mapping(task=data_args.task)
olmo_config.label2id = label2id
...
print("Vocab size of Config before tokenization: ", olmo_config.vocab_size)
print("Vocab size of Tokenizer before tokenization: ", len(tokenizer))

# Load OLMo model
olmo = AutoModelForCausalLM.from_pretrained(pretrained_model_name_or_path=model_args.model_path, **olmo_config_kwargs)
